"""MCP session setup — allowlist → admit → connect → register (Spec 06 slice 4).
...
It lives in :mod:`dream.mcp` (not the REPL) so any front end — the REPL loop
*and* ``build_harness``'s async opener — can drive it without depending on the
REPL. It takes an optional ``session_opener`` so it can be driven over the
in-memory transport in tests without spawning real servers.
...
from dream.mcp._allowlist import AllowlistError, read_allowlist
from dream.mcp._client import McpClientManager, SessionOpener
...
async def setup_mcp_session(
    ...
    *,
    ...
    # by ``connect_all`` would leak and the exception would abort the session.
    try:
        registered = (
