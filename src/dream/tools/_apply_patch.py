"""Codex/OpenAI ``apply_patch`` text format — pure parse + apply.

Adapted from the OpenAI cookbook / OpenHands reference (GPT-5 apply_patch
guide). File I/O is injected so callers can enforce workspace confinement and
atomic writes. No tool/engine imports.
"""
...
def peek_next_section(
    ...
    while index < len(lines):
        ...
        if s[0] == "+":
            ...
            raise DiffError(f"Invalid Line: {s}")
        s = s[1:]
        if mode == "keep" and last_mode != mode and (ins_lines or del_lines):
            chunks.append(
                Chunk(
...
def identify_files_added(text: str) -> list[str]:
    ...
    for line in text.strip().split("\n"):
        if line.startswith("*** Add File: "):
            result.append(line[len("*** Add File: ") :])
    return result
...
def process_patch(
    ...
    """Parse ``text`` and apply via I/O callables. Returns (message, fuzz, commit)."""
    stripped = text.strip()
